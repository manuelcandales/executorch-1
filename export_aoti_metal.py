...
class AddStridedSDPA(nn.Module):
    """
    SDPA model with Q, K, V as parameters that adds an input to the SDPA output.
    Query, Key, Value are model parameters. Takes one input and adds it to SDPA result.
    """
    def __init__(self, batch_size=10, num_heads=20, seq_len=1500, head_dim=64, strides=[1920000, 64, 1280, 1]):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query = torch.as_strided(self.query, size=[10, 20, 1500, 64], stride=self.strides)
        key = torch.as_strided(self.key, size=[10, 20, 1500, 64], stride=self.strides)
        value = torch.as_strided(self.value, size=[10, 20, 1500, 64], stride=self.strides)
...
MODEL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "audio_encoder_sdpa1": {
        ...
        "description": "Audio Encoder model with SDPA1",
    },
}
